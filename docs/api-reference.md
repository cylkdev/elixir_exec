...
- **Result structs** — [`%ElixirExec.Stream{}`](#elixirexecosprocess), [`%ElixirExec.Output{}`](#elixirexecoutput)
...
@spec run(command(), command_options()) ::
        {:ok, ElixirExec.Stream.t()} | {:ok, ElixirExec.Output.t()} | {:error, term()}
def run(command, options \\ [])
```

Starts an external command. `sync: true` blocks until exit and returns `%Output{}` with captured stdout/stderr. Without `sync: true`, returns immediately with `%Stream{}` carrying the `:controller` pid and `:os_pid`.

Returns:

- `{:ok, %ElixirExec.Stream{}}` — async start.
...
@spec stream(command(), command_options()) ::
        {:ok, ElixirExec.Stream.t()} | {:error, term()}
```

Convenience wrapper that forces `monitor: true` and `stdout: :stream`, then calls `run/2`. The returned `%Stream{}.stream` is an `Enumerable` over each line written to stdout (newline kept). Works with `Enum`, `Stream`, `Enum.take/2`, and other early-termination patterns.
...
Pulls **the next** matching message for `os_pid`. Decodes `:DOWN` reasons via `Stream.decode_reason/1` (so a normal exit becomes `{:exit, 0}`, an exit-status tuple becomes `{:exit, n}`, and anything else passes through). Non-matching mailbox messages are not consumed.
...
`ElixirExec.Stream.decode_reason/1` is exposed as a helper:
